...
router.post('/temporary-assignment', requireSupervisor, async (req, res) => {
    try {
        ...
        // This endpoint only records *that* the technician is temporarily borrowed
        // (the TemporaryAssignment doc above). Assigning them to work is a separate,
        // deliberate action the supervisor takes next — either creating a new job with
        // them included, or adding them to one specific existing job via the job's own
        // "assign technician" action. Both of those already scope correctly to a single
        // job; this endpoint must not also inject the technician into every job in the
        // workshop, or they'd end up assigned to unrelated jobs they were never given.
